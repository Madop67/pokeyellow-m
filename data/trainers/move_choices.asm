DEF __move_choices__ = 0

MACRO move_choices
	IF _NARG
		db \# ; all args
	ENDC
	db 0 ; end
	DEF __move_choices__ += 1
ENDM

; move choice modification methods that are applied for each trainer class
; layers 1, 3: fundamentals (no wasted status moves, full type awareness)
; layer 2: legacy setup-move preference (kept for flavor classes)
; layers 4, 5: boss tier (expected-damage move selection + early planning);
;              layer 4 subsumes layer 3's type scoring, so bosses don't list 3
TrainerClassMoveChoiceModifications:
	move_choices 1, 3    ; YOUNGSTER
	move_choices 1, 3    ; BUG CATCHER
	move_choices 1, 3    ; LASS
	move_choices 1, 3    ; SAILOR
	move_choices 1, 3    ; JR_TRAINER_M
	move_choices 1, 3    ; JR_TRAINER_F
	move_choices 1, 2, 3 ; POKEMANIAC
	move_choices 1, 2, 3 ; SUPER_NERD
	move_choices 1, 3    ; HIKER
	move_choices 1, 3    ; BIKER
	move_choices 1, 3    ; BURGLAR
	move_choices 1, 3    ; ENGINEER
	move_choices 1, 2, 3 ; UNUSED_JUGGLER
	move_choices 1, 3    ; FISHER
	move_choices 1, 3    ; SWIMMER
	move_choices 1, 3    ; CUE_BALL
	move_choices 1, 3    ; GAMBLER
	move_choices 1, 3    ; BEAUTY
	move_choices 1, 2, 3 ; PSYCHIC_TR
	move_choices 1, 3    ; ROCKER
	move_choices 1, 3    ; JUGGLER
	move_choices 1, 3    ; TAMER
	move_choices 1, 3    ; BIRD_KEEPER
	move_choices 1, 3    ; BLACKBELT
	move_choices 1, 4, 5 ; RIVAL1
	move_choices 1, 4, 5 ; PROF_OAK
	move_choices 1, 2, 3 ; CHIEF
	move_choices 1, 2, 3 ; SCIENTIST
	move_choices 1, 4, 5 ; GIOVANNI
	move_choices 1, 3    ; ROCKET
	move_choices 1, 3, 4 ; COOLTRAINER_M
	move_choices 1, 3, 4 ; COOLTRAINER_F
	move_choices 1, 4, 5 ; BRUNO
	move_choices 1, 4, 5 ; BROCK
	move_choices 1, 4, 5 ; MISTY
	move_choices 1, 4, 5 ; LT_SURGE
	move_choices 1, 4, 5 ; ERIKA
	move_choices 1, 4, 5 ; KOGA
	move_choices 1, 4, 5 ; BLAINE
	move_choices 1, 4, 5 ; SABRINA
	move_choices 1, 2, 3 ; GENTLEMAN
	move_choices 1, 4, 5 ; RIVAL2
	move_choices 1, 4, 5 ; RIVAL3
	move_choices 1, 4, 5 ; LORELEI
	move_choices 1, 3    ; CHANNELER
	move_choices 1, 4, 5 ; AGATHA
	move_choices 1, 4, 5 ; LANCE
	assert __move_choices__ == NUM_TRAINERS, \
		"TrainerClassMoveChoiceModifications: expected {d:NUM_TRAINERS} entries, got {d:__move_choices__}"
